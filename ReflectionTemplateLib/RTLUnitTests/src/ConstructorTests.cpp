...
const vector<TestFunctor> ConstructorTests::UNIT_TEST_CASES = {
	...
	____REFLECTION_UNIT_TEST_CASE____
	{
		...
		fact.ASSERT_SIGNATURE(bookClass.isCtorSignature<>());

		auto book = unique_ptr<Book>(new Book());
		auto reflBook = bookClass.instance();
	...
	____REFLECTION_UNIT_TEST_CASE____
	{
		...
		auto reflBook = bookClass.instanceConst();
	...
	____REFLECTION_UNIT_TEST_CASE____
	{
		...
		fact.ASSERT_SIGNATURE(bookClass.isCtorSignature<ctor::COPY>());

		auto book = unique_ptr<Book>(new Book(*GET_BOOK_INSTANCE().get()));
		auto reflBookSrc = ReflObject<>::create(GET_BOOK_INSTANCE());
		auto reflBook = bookClass.instance<ctor::COPY>(reflBookSrc);
	...
	____REFLECTION_UNIT_TEST_CASE____
	{
		...
		fact.ASSERT_SIGNATURE(bookClass.isCtorSignature<double, string>());

		auto book = unique_ptr<Book>(new Book(BOOK_PRICE, string(BOOK_TITLE)));
		auto reflBook = bookClass.instance(BOOK_PRICE, string(BOOK_TITLE));
	...
	____REFLECTION_UNIT_TEST_CASE____
	{
		...
		fact.ASSERT_SIGNATURE(bookClass.isCtorSignature<const string&>());
...
*/		auto reflBook = bookClass.instance<void, const string&>(BOOK_TITLE);
	...
	____REFLECTION_UNIT_TEST_CASE____
	{
		...
		fact.ASSERT_SIGNATURE(bookClass.isCtorSignature<Date&, string>());

		auto date = GET_DATE_INSTANCE();
		auto book = unique_ptr<Book>(new Book(date, string(BOOK_TITLE)));
		auto reflBook = bookClass.instance<void, Date&>(date, string(BOOK_TITLE));
